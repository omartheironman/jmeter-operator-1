remote_state {
  ...
  config = {
    bucket   = "${get_env("TF_VAR_project", "unity-cs-recorderjmeter-test")}-terraform-tg"
    prefix   = "${get_env("TF_VAR_region", "us-central1")}/${path_relative_to_include()}"
    project  = "${get_env("TF_VAR_project", "unity-cs-recorderjmeter-test")}"
    location = "us"
    gcs_bucket_labels = {
      business_unit = "cloudservices"
      role          = "terraform_state"
      product       = "devops_performance"
    }
  }
}
